...
gh_ensure_release() {
  ...
  if [[ -z "$id" ]]; then
    if [[ -n "$SYNC_AUTH_TOKEN" ]]; then
      ...
      log "Release '${GH_RELEASE_TAG}' not found and no token -> RO skip"
      GH_REL_ID=""
      return 0
    fi
  fi
...
openssl req \
  -newkey rsa:2048 \
  -nodes \
  -keyout "${CONFIG_PATH}/hexlicsrv.key" \
  -out "${CONFIG_PATH}/hexlicsrv.csr" \
  -subj "/CN=${LICENSE_HOST}" \
  -config "$OPENSSL_CFG" \
  -reqexts v3_req \
...
openssl x509 \
  -req \
  -in "${CONFIG_PATH}/hexlicsrv.csr" \
  -CA "${CA_PATH}/CA.pem" \
  -CAkey "${CA_PATH}/CA.key" \
  -CAcreateserial \
  -out "${CONFIG_PATH}/hexlicsrv.crt" \
  -days 365 \
  -sha512 \
  -extensions v3_req \
  -extfile "$OPENSSL_CFG" \
