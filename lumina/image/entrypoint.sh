...
ensure_secret_service_bus() {
  ...
  mkdir -p "$XDG_RUNTIME_DIR" /run/lumina-dbus
  chmod 700 "$XDG_RUNTIME_DIR"
  chmod 755 /run/lumina-dbus
...
assemble_remote_archive() {
  ...
  ((${#parts[@]} > 0)) || die "Remote DB parts not found"
...
perform_commits_sync() {
  ...
  if [[ "$GH_MODE" == "HTTPS_PULLONLY" ]]; then
    if [[ -n "$man" ]]; then
      log "RO: force-restore DB snapshot from repo"
      restore_from_remote
    else
      log "RO: no DB snapshot in repo -> keep local"
    fi
    return 0
  fi

  if db_is_empty && [[ -n "$man" ]]; then
    log "DB empty -> restore from repo snapshot"
    restore_from_remote
    return 0
  fi
  ...
  if [[ "$need_push" == "yes" ]]; then
    ...
    git -C "$WORK_DIR" commit -m "mysql-backup(${SYNC_HOST_ID}/${MYSQL_DATABASE}): size=${size} sha256=${sha}"
