...
chown hexvault:hexvault "$CONFIG_FILE" "${CONFIG_PATH}/hexvault.crt" "${CONFIG_PATH}/hexvault.key" "${INSTALL_PATH}/teams_server.hexlic" || true
...
exec "${INSTALL_PATH}/vault_server" -f "$CONFIG_FILE" -p "$VAULT_PORT" -l "${LOGS_PATH}/vault_server.log" -c "${CONFIG_PATH}/hexvault.crt" -k "${CONFIG_PATH}/hexvault.key" -L "${INSTALL_PATH}/teams_server.hexlic" -d "$DATA_PATH"
