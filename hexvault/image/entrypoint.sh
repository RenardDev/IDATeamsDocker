...
GH_WORK="${INSTALL_PATH}/_gitmirror"
REMOTE_DIR="${GH_WORK}/backups/${GH_HOST_ID:-hexvault}"
...
if [[ ! -f "$CONFIG_FILE" ]]; then echo "sqlite3;Data Source=${DATA_PATH}/hexvault.sqlite3;" > "$CONFIG_FILE"; chmod 640 "$CONFIG_FILE"; fi
...
if [[ ! -f "$SCHEMA_LOCK" ]]; then
  if [[ "$SKIP_SCHEMA_RECREATE" -eq 0 ]]; then "${INSTALL_PATH}/vault_server" -f "$CONFIG_FILE" -d "$DATA_PATH" --recreate-schema; fi
  touch "$SCHEMA_LOCK"
fi
