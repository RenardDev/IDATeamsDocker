...
GH_RELEASE_TAG="${GH_RELEASE_TAG:-hexvault-snapshot}"
GH_RELEASE_NAME="${GH_RELEASE_NAME:-HexVault FS Snapshot}"
GH_API="${GH_API:-}"
GH_UPLOAD="${GH_UPLOAD:-}"

################################################################
# Utils (same as hexlicsrv)
################################################################
...
ensure_tools_common() { local m=(); for t in tar zstd jq sha256sum split openssl; do command -v "$t" >/dev/null 2>&1 || m+=("$t"); done; ((${#m[@]}==0)) || die "Missing tools: ${m[*]}"; }

################################################################
# Payload (FS)
################################################################
pack_payload() { rm -f "$ARCHIVE_PATH"; mkdir -p "$DATA_PATH"; log "Packing FS -> $ARCHIVE_PATH" >&2; tar -C "$DATA_PATH" -cf - . | zstd -q -T0 -19 -o "$ARCHIVE_PATH"; local size sha; size="$(stat -c '%s' "$ARCHIVE_PATH")"; sha="$(sha256sum "$ARCHIVE_PATH" | awk '{print $1}')"; printf '%s %s\n' "$size" "$sha"; }
import_payload() { rm -rf "${DATA_PATH:?}/"* "${DATA_PATH}/."[!.]* 2>/dev/null || true; mkdir -p "$DATA_PATH"; tar -C "$DATA_PATH" -xpf "$1"; }

################################################################
# Chunk helpers
################################################################
split_archive_into_remote() { local bs=$((SYNC_CHUNK_SIZE_MB * 1000000)); mkdir -p "$REMOTE_DIR"; rm -f "${REMOTE_DIR}/${ARCHIVE_NAME}.part_"* "${REMOTE_DIR}/${MANIFEST_NAME}" || true; log "Splitting archive to ${REMOTE_DIR} by ${SYNC_CHUNK_SIZE_MB}MB"; split -b "$bs" -d -a 3 "$ARCHIVE_PATH" "${REMOTE_DIR}/${ARCHIVE_NAME}.part_"; }
assemble_remote_archive() { local dest="$1"; rm -f "$dest"; local parts=( "${REMOTE_DIR}/${ARCHIVE_NAME}.part_"* ); ((${#parts[@]} > 0)) || die "Remote parts not found"; cat "${parts[@]}" > "$dest"; }
write_manifest() {
  local ts="$1" size="$2" sha="$3"; local parts=( "${REMOTE_DIR}/${ARCHIVE_NAME}.part_"* ); local cnt="${#parts[@]}"
  jq -n --arg host_id "${SYNC_HOST_ID}" --arg timestamp_utc "$ts" --arg chunk_size_mb "${SYNC_CHUNK_SIZE_MB:-0}" --arg archive_size_bytes "${size:-0}" --arg archive_sha256 "$sha" --arg chunk_count "${cnt:-0}" '
...
restore_from_remote() {
  ...
  import_payload "$tmp/${ARCHIVE_NAME}"; rm -rf "$tmp"; SKIP_SCHEMA_RECREATE=1; log "Restore completed (sha=$sha_remote)"
}

################################################################
# Commits mode (same logic)
################################################################
...
perform_commits_sync() {
  ...
  if [[ "$GH_MODE" == "HTTPS_PULLONLY" ]]; then if [[ -n "$man" ]]; then log "RO: force-restore FS snapshot from repo"; restore_from_remote; else log "RO: no snapshot in repo -> keep local"; fi; return 0; fi
  ...
  if [[ "$need_push" == "yes" ]]; then split_archive_into_remote; write_manifest "$(now_utc)" "$size" "$sha"; git -C "$WORK_DIR" add -A; if git -C "$WORK_DIR" diff --cached --quiet; then log "Nothing to commit"; return 0; fi; git -C "$WORK_DIR" commit -m "fs-backup(${SYNC_HOST_ID}): size=${size} sha256=${sha}"; git -C "$WORK_DIR" push origin "$GH_BRANCH"; log "Pushed FS snapshot commit"; fi
}

################################################################
# Releases mode (same as hexlicsrv)
################################################################
...
GH_OWNER="" GH_REPO=""
parse_gh_remote() { [[ -n "$GH_REMOTE" ]] || die "GH_REMOTE is required for releases mode"; local url="$GH_REMOTE" tmp host path; if [[ "$url" =~ ^https?:// ]]; then tmp="${url#*://}"; host="${tmp%%/*}"; path="${tmp#*/}"; elif [[ "$url" =~ ^ssh://([^/]+)/(.+)$ ]]; then host="${BASH_REMATCH[1]}"; host="${host#*@}"; path="${BASH_REMATCH[2]}"; elif [[ "$url" =~ ^[^@]+@([^:]+):(.*)$ ]]; then host="${BASH_REMATCH[1]}"; path="${BASH_REMATCH[2]}"; else die "Unsupported GH_REMOTE: $GH_REMOTE"; fi; path="${path%.git}"; GH_OWNER="${path%%/*}"; GH_REPO="${path#*/}"; [[ -n "$GH_OWNER" && -n "$GH_REPO" && "$GH_REPO" != "$GH_OWNER" ]] || die "Cannot parse owner/repo"; if [[ -z "$GH_API" ]]; then [[ "$host" == "github.com" ]] && GH_API="https://api.github.com" || GH_API="https://${host}/api/v3"; fi; if [[ -z "$GH_UPLOAD" ]]; then [[ "$host" == "github.com" ]] && GH_UPLOAD="https://uploads.github.com" || GH_UPLOAD="https://${host}/api/uploads"; fi; log "Releases: parsed host=${host} owner=${GH_OWNER} repo=${GH_REPO}"; }
...
if [[ ! -f "$CONFIG_FILE" ]]; then echo "sqlite3;Data Source=${DATA_PATH}/hexvault.sqlite3;" > "$CONFIG_FILE"; chmod 640 "$CONFIG_FILE"; log "Created default config $CONFIG_FILE"; fi
...
log "Patching license"; python3 "${INSTALL_PATH}/license_patch.py" hexvault || die "Patch failed"

chown root:root "${INSTALL_PATH}/vault_server" || true
chmod 755 "${INSTALL_PATH}/vault_server" || true

if [[ ! -f "$SCHEMA_LOCK" ]]; then
  if [[ "$SKIP_SCHEMA_RECREATE" -eq 0 ]]; then log "Recreating schema..."; "${INSTALL_PATH}/vault_server" -f "$CONFIG_FILE" -d "$DATA_PATH" --recreate-schema
  else log "Schema recreate skipped (restored from snapshot)"; fi
  touch "$SCHEMA_LOCK"
fi

log "Generating TLS cert via local CA"
openssl req -newkey rsa:2048 -keyout "${CONFIG_PATH}/hexvault.key" -out "${CONFIG_PATH}/hexvault.csr" -nodes -subj "/CN=${VAULT_HOST}" >/dev/null 2>&1 || die "CSR failed"
...
chown_if_user hexvault \
  "$CONFIG_FILE" "${CONFIG_PATH}/hexvault.crt" "${CONFIG_PATH}/hexvault.key" \
