...
2. Copy the same `CA.pem` and `CA.key` pair into all three directories:
...
4. Configure service secrets:
   ...
   - Set `VAULT_PASSWORD` for the fixed HexVault admin user `hexvault`. It is used by the first-login bootstrap and then by `--set-admin` before the normal server starts.
   - Set strong values for `MYSQL_PASSWORD` and `MYSQL_ROOT_PASSWORD`. `MYSQL_PASSWORD` must not contain `;`, which Lumina reserves as a connection-string delimiter.
...
- An unmarked HexLicSrv database is adopted only when its canonical schema fingerprint exactly matches a disposable current-version schema generated by the same image. Older unmarked, modified and foreign schemas remain rejected; automatic version inference and blind upgrades are disabled.
...
- HexVault uses the fixed account `hexvault`. `VAULT_PASSWORD` is required only for a fresh schema. After recreation, the patched official `hv` client performs the first login through a loopback-only bootstrap listener, then the stopped server runs `--set-admin hexvault:PASSWORD`. There is no independent password-reset lock.
...
Remote sync covers HexLicSrv data, the complete HexVault `data/` tree (including regular user files and directories), and the Lumina MySQL database. At the top level of HexVault `data/`, `store`, SQLite files/sidecars, schema identity and internal rollback prefixes remain reserved service names; other regular files and directories are user payload. Symlinks, special files and nested mount points are rejected instead of being followed into a snapshot. Persistent `config/`, `keyrings/` and service recovery state are deliberately separate operational data; include them in the host's normal encrypted filesystem backup. Recovery state keeps the original data until automatic recovery succeeds; if automatic rollback also fails, the emergency copy is preserved for manual recovery. `CA.key` is never included in GitHub snapshots.
...
- Missing CA files: check `CA/CA.pem` and `CA/CA.key`. Both should be regular files before Compose starts, and the same pair must be used by HexLicSrv, HexVault and Lumina so that all client/server patch profiles share one trust root.
